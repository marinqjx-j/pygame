...
# Forest Setup - spawne 1 Wood + 1 Stone zum Aufheben (für erste Axt)
if current_room == 4 and not axe_crafted:  # Forest room, noch keine Axt
    # Prüfe ob Wood/Stone schon auf dem Boden liegen
    has_wood = any(item['type'] == ITEM_WOOD for item in dropped_items)
    has_stone = any(item['type'] == ITEM_STONE for item in dropped_items)
    
    if not has_wood:
        dropped_items.append({
            'type': ITEM_WOOD,
            'rect': wood_img.get_rect(topleft=(300, 450)),
            'img': wood_img
        })
    if not has_stone:
        dropped_items.append({
            'type': ITEM_STONE,
            'rect': stone_img.get_rect(topleft=(400, 450)),
            'img': stone_img
        })
