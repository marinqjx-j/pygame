...
def render_inventory(surface, mouse_pos, inventory, equipped, throw_cd_val=0):
    ...
    if throw_cd > 0 and equipped < len(rects):
        ...
        if eq_slot and eq_slot.get("type") in (ITEM_KNIFE, ITEM_SPIKE):
            ...
            cd_txt = small_font.render(f"{math.ceil(throw_cd / 60 * 10) / 10:.1f}", True, (255, 255, 180))
            surface.blit(cd_txt, (r.centerx - cd_txt.get_width() // 2, r.centery - cd_txt.get_height() // 2))
...
while run:
    ...
    if gs in ("main", "boss_fight"):
        ...
        if state["scorpion_active"]:
            ...
            if _hard:
                ...
                state["scorpion_move_timer"] = state.get("scorpion_move_timer", 0) - 1
                ...
                if dist > 120:
                    state["scorpion_rect"].x += int(state["scorpion_strafe_dx"] * spd_sc)
                    state["scorpion_rect"].y += int(state["scorpion_strafe_dy"] * spd_sc)
                elif dist < 60:
                    state["scorpion_rect"].x -= int(dx / dist * 1.5)
                    state["scorpion_rect"].y -= int(dy / dist * 1.5)
                state["scorpion_rect"].clamp_ip(pygame.Rect(0, 0, WIDTH, HEIGHT))
        ...
        if state["lala_alive"]:
            if _hard:
                ...
                if pdist2 > 160:
                    ...
                    state["lala_rect"].x += int(state.get("lala_move_dx", 0) * lala_spd)
                    state["lala_rect"].y += int(state.get("lala_move_dy", 0) * lala_spd)
    ...
    if gs == "name_input":
        ...
        em_label = "✔ EASY MODE  (5 lives, slower enemies)" if easy else "☐ HARD MODE  (3 lives, dynamic enemies)"
